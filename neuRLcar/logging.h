...
{
	// First create a format_arg_store containing *lvalue* arguments
	auto store = std::make_format_args(args...);

	// Then use vformat(), which does NOT require compile-time format literals
	auto text = std::vformat(fmt, store);

	_globalCvarManager->log(text);
}
